...
export class FileListComponent implements OnInit {
  ...
  // Updated onSelectFile to check analysis type through file name suffix
  onSelectFile(file: FileSystemEntity): void {
    const analysisType = this.getAnalysisTypeFromFile(file.name);
    if (analysisType) {
      this.getAnalysisResult(file, analysisType);
    } else {
      console.log("Selected file is not an analysis result.");
    }
  }

  // Helper function to determine analysis type based on file name suffix
  getAnalysisTypeFromFile(fileName: string): string | null {
    if (fileName.endsWith("_wordcount")) return "wordcount";
    if (fileName.endsWith("_kmeans")) return "kmeans";
    if (fileName.endsWith("_tfidf")) return "tfidf";
    if (fileName.endsWith("_network")) return "network";
    if (fileName.endsWith("_ngrams")) return "ngrams";
    if (fileName.endsWith("_w2v")) return "w2v";
    if (fileName.endsWith("_hc")) return "hc";
    if (fileName.endsWith("_lda")) return "lda";
    if (fileName.endsWith("_ner")) return "ner";
    return null; // Return null if no matching suffix is found
  }

  // getAnalysisResult now accepts analysisType as a parameter
  getAnalysisResult(file: FileSystemEntity, analysisType: string): void {
    ...
    this.middlewareService.getAnalysisResult(params).subscribe(
      (response) => {
        if (response.success) {
          this.analysisedData = response.file_content;
          console.log("Analysis result:", this.analysisedData);

          // Use analysisType directly
          switch (analysisType) {
            case "wordcount":
              this.drawBarChart(this.analysisedData);
              break;
            case "kmeans":
              this.drawScatterChart(this.analysisedData);
              break;
            case "tfidf":
            case "count":
              this.drawTable("count", this.analysisedData);
              this.drawBarChart(this.analysisedData);
              break;
            case "network":
            ...
            case "w2v":
              ...
              break;
            case "hc":
              this.drawTreeChart(this.analysisedData);
              break;
            case "lda":
              this.drawTopicModeling(this.analysisedData);
              break;
            case "ner":
              this.drawTable("ner", this.analysisedData);
              break;
            default:
              console.error("Unknown analysis type.");
          }
        } else {
          console.error("Failed to load analysis result:", response.message);
        }
      },
      (error) => {
        console.error("Error fetching analysis result:", error);
